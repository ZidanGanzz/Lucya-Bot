...
module.exports = msgHndlr = async (aksa, message) => {
    try {
        ...
        const vhtear = 'ZidanGanzz' // apikey from vhtear
		...
		try {
			...
			aksa.sendAudio(from, `data:audio/mp3;base64,${getvn.toString('base64')}`, id)
		} catch {
			aksa.reply(from, 'Maaf, sistem error', id)
		}
        ...
        if (args.includes('@6285728570938')) { //replace with your bot number
...
Contoh: ${prefix}setprefix #`, id)
				...
				if (quotedMsg && quotedMsg.type == 'sticker' || quotedMsg && quotedMsg.type == 'image') {
                        ...
                        await aksa.sendImageAsSticker(from, imageBase64, { author: `${authors}`, pack: `${packnames}` })
                        .catch(async (err) => {
                            console.error(err)
						...
						aksa.reply(from, 'format pesan salah kak', id)
					}
        break
                case `${prefix}unblock`:
                    if (!isOwner) return aksa.reply(dari, 'Perintah ini hanya bisa digunakan oleh Owner', id)
                ...
                case `${prefix}join`:
                  //  if (!isOwner) return aksa.reply(dari, 'Silakan hubungi Owner untuk mengundang Lucya ke dalam Grup. Ketik !owner', id)
                    if (args.length < 2) return aksa.reply(dari, `Contoh: ${prefix}join https://chatt.whatsapp.com/xnxx`, id)
                    const link = args[1]
                   ...
                   aksa.reply(dari, `Okey lucya akan join. Terimakasih telah mengundangku kak *${pushname}*`, id)
...
*Provider :* INDIHOME ASU`, id)
                ...
                const mediaData = await decryptMedia(message, uaOverride)
                const imageBase64 = `data:${mimetype};base64,${mediaData.toString('base64')}`
                await aksa.sendImageAsSticker(from, imageBase64, StickerMetadata)
...
*➸ Group Description : ${desc}*`)
                ...
                case `${prefix}pencil`:
                    if (isMedia || isImage || isQuotedImage) {
						...
						await aksa.sendFileFromUrl(from, `https://videfikri.com/api/textmaker/pencil/?urlgbr=${beimages}`, '', '', id)
						.catch(() => {
							aksa.reply(from, 'Kesalahan waktu mengupload foto, silahkan coba lagi', id)
						})
					} else {
						aksa.reply(from, 'Format pesan salah, silahkan post/reply foto', id)
					}
					break
                case `${prefix}heroml`:
                    if (!isGroupMsg) return aksa.reply(dari, 'Perintah ini hanya bisa di gunakan dalam group!', id)
...
➸ *Cara:* ${cara}`
                ...
                case `${prefix}tutup`:
                case `${prefix}close`:
                    if (args.length !== 1) return aksa.reply(from, `Untuk mengubah settingan group chat agar hanya admin saja yang bisa chat\n\nPenggunaan:\n#tutupgrub on --aktifkan\n#tutupgrub off --nonaktifkan`, id)
                    if (!isRegis) return aksa.reply(dari, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
                    if (isLimit(serial)) return aksa.reply(dari, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)

                    await limitAdd(serial)
                    const oranggg = args[1]
                    if (!isGroupMsg) return aksa.reply(dari, 'Fitur ini hanya bisa digunakan dalam group', id)
			    aksa.setGroupToAdminsOnly(groupId, true).then(() => aksa.sendText(from, 'Berhasil mengubah agar hanya admin yang dapat chat!'))
                if (!isGroupAdmins) return aksa.reply(dari, 'Perintah ini hanya bisa digunakan oleh admin group', id)
                    if (!isBotGroupAdmins) return aksa.reply(dari, 'Perintah ini hanya bisa digunakan ketika Lucya menjadi admin', id)
                break
                case `${prefix}unlock`:
                ...
                case `${prefix}open`:
                    ...
                    const orangg = args[1]
                    if (!isGroupMsg) return aksa.reply(dari, 'Fitur ini hanya bisa digunakan dalam group', id)
                    aksa.setGroupToAdminsOnly(groupId, false).then(() => aksa.sendText(from, 'Berhasil mengubah agar semua anggota dapat chat!'))
                if (!isGroupAdmins) return aksa.reply(dari, 'Perintah ini hanya bisa digunakan oleh admin group', id)
                    if (!isBotGroupAdmins) return aksa.reply(dari, 'Perintah ini hanya bisa digunakan ketika Lucya menjadi admin', id)
                break
                case `${prefix}add`:
...
➸ *Type:* ${Type}`
				...
				case `${prefix}listvn`:
							...
							kemtul += '╚═〘 *Lucya Bot* 〙'
							await aksa.reply(from, kemtul, id)
							break
			case `${prefix}addvn`:
			case `${prefix}setvn`:
				...
				if (quotedMsg && quotedMsg.type === 'audio' || quotedMsg && quotedMsg.type === 'ptt') {
					...
					aksa.reply(from, `vn dengan nama ${nuhi} berhasil disimpan didalam database!`, id)
				} else if(isMedia && type === 'audio' || isMedia && type === 'ptt') {
					var mediaData = await decryptMedia(message, uaOverride)
					var filename = `./media/audio/${nuhi}.mp3`
					await fs.writeFileSync(filename, mediaData)
					await aksa.reply(from, `vn dengan nama ${nuhi} berhasil disimpan didalam database!`, id)
				} else {
					return aksa.reply(from, `Error! silahkan coba kembali...`, id)
				}
				listvn.push(nuhi)
				fs.writeFileSync('./lib/database/listvn.json', JSON.stringify(listvn))
				break
			case `${prefix}delallvn`:
			if (!isOwner) return aksa.reply(from, 'Owner Only', id)
			let dellall = listvn.includes(chats)
			listvn.splice(dellall)
			fs.writeFileSync('./lib/database/listvn.json', JSON.stringify(listvn))
			aksa.reply(from, `semua vn didalam database berhasil dihapus`, id)
			break
			case `${prefix}delvn`:
			let deli = listvn.indexOf(body.slice(7))
				listvn.splice(deli, 1)
				fs.writeFileSync('./lib/database/listvn.json', JSON.stringify(listvn))
				aksa.reply(from, 'vn berhasil didelete dari database', id)
				break
                case `${prefix}ytsearch`:
                case `${prefix}searchyt`:
				case `${prefix}cariyt`:
                    if (args.length === 1) return aksa.reply(from, `Kirim perintah *${prefix}ytsearch [ Query ]*, Contoh : #ytsearch alan walker alone`)
            ...
            try {
                ...
                await aksa.sendFileFromUrl(from, result[0].image, 'thumbserc.jpg', xixixi, id)
                await limitAdd(serial)
            } catch (err) {
                    console.log(err)
                    await aksa.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, Video tidak ditemukan')
                    aksa.sendText(ownerNumber, 'YT Search Error : ' + err)
            }
            break
                case `${prefix}play`:
                    if (!isRegis) return aksa.reply(dari, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
...
• *Deskripsi:* ${desk}`
		...
		aksa.sendFileFromUrl(from, hekel, id)
		break
                case `${prefix}images`:
                    if (!isRegis) return aksa.reply(dari, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
                    if (isLimit(serial)) return aksa.reply(dari, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
                    ...
                    aksa.sendFileFromUrl(dari, hasilwall, '', `Jangan lupa donasi ya kak *${pushname}*. jika minat kirim *${prefix}donasi*.`, id)
                        .catch(() => {
                            aksa.reply(dari, 'Ada yang eror!', id)
                        })
                    break
...
➸ *Lirik: *${lirik}`
				...
				case `${prefix}tik`:
				case `${prefix}tt`:
				    if (!isOwner) return aksa.reply(dari, 'fitur ini dinonaktifkan oleh owner', id)
                    if (!isRegis) return aksa.reply(dari, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
                    if (isLimit(serial)) return aksa.reply(dari, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
                    ...
                    if (isMedialimit(serial)) return aksa.reply(dari, `Maaf ${pushname}, kuota limit media kamu sudah habis, ketik !limed untuk mengecek kuota Limit media kamu`, id)
                    await MedialimitAdd(serial)
                    if (args.length === 1) return aksa.reply(dari, `Link nya mana kak *${pushname}*`, id)
                    const tiktod = await axios.get(`https://api.areltiyan.xyz/tiktokwm?link=${body.slice(12)}`)
                        await aksa.sendFileFromUrl(from, tiktod.data.no_watermark_link, '', `Yeay berhasil! nih videonya jangan lupa donasi ya *${pushname}* jika minat silahkan ketik *${prefix}donasi* `, id)
                    break
                case `${prefix}igstalk`:
                    if (!isRegis) return aksa.reply(dari, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
                    if (isLimit(serial)) return aksa.reply(dari, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
...
Twitter: https://twitter.com/bdrsmsdn`)
                ...
                case `${prefix}owner`:
                    if (!isRegis) return aksa.reply(dari, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
                    aksa.sendTextWithMentions(dari, `My Owner @6281310253704@c.us`, id) //ganti
                    break
                case `${prefix}bot1`:
                    if (!isRegis) return aksa.reply(dari, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
                    aksa.sendTextWithMentions(dari, `Nih nomo nya @6282115089860@c.us`) //bot gue ini
                    break
                case `${prefix}bot2`:
                    if (!isRegis) return aksa.reply(dari, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
                    aksa.sendTextWithMentions(dari, `Nih nomor nya @6285215130891@c.us`) //ini juga
                    break
